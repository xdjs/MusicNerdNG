...
function Users({
    ...
    function navigateToUser(artist: SpotifyArtist) {
                        ...
                        >
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    {u.genres && u.genres.length > 0 && (
                                        <div className="text-xs text-gray-500 truncate">
                                            {u.genres.slice(0, 2).join(', ')}
                                        </div>
                                    )}
                                </div>
                            </div>
...
const SearchBar = ({isTopSide}: {isTopSide: boolean}) => {
    ...
    const { data, isLoading, isFetching } = useQuery({
        queryKey: ['spotifyArtistSearch', debouncedQuery],
        queryFn: async () => {
            if (!debouncedQuery) return { artists: [], total: 0, error: null };
            return await searchSpotifyArtists(debouncedQuery);
        },
    ...
    return (
        <div className="relative w-full max-w-[400px] z-30 text-black">
            ...
            {(showResults && query.length >= 1) && (
                <div ref={resultsContainer} className={`absolute left-0 w-full mt-2 bg-white rounded-lg shadow-2xl max-h-80 overflow-y-auto px-1 py-1 scrollbar-hide ${isTopSide ? "bottom-14" : "top-12"}`}>
                    {isLoading ? <Spinner />
                        :
                        <>
                            {data &&
                                <Users users={data.artists} search={search ?? ""} setQuery={setQuery} />
                            }
                        </>
                    }
                </div>
