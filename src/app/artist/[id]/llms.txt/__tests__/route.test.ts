...
const getArtistDoc = jest.fn();
const getArtistById = jest.fn();

jest.mock('@/server/utils/queries/onboardingQueries', () => ({ getArtistDoc: (...a) => getArtistDoc(...a) }));
...
describe('GET /artist/[id]/llms.txt', () => {
    ...
    it('serves the document to nobody in particular — no session required', async () => {
        getArtistDoc.mockResolvedValue({
    ...
    it('says how stale it is, because a crawler cannot tell', async () => {
        getArtistDoc.mockResolvedValue({ content: 'x', sources: [], updatedAt: '2026-09-03T10:00:00Z' });
    ...
    it('404s rather than serving an empty document', async () => {
        ...
        getArtistDoc.mockResolvedValue({ content: '', sources: [] });
        const res = await get();
    ...
    it('degrades to 503 rather than throwing', async () => {
        getArtistDoc.mockRejectedValue(new Error('db down'));
