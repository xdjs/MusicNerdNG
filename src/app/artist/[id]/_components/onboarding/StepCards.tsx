...
// ---------- Identity: the hybrid flow's single opening question ----------
...
/**
 * One card, one question, answerable by looking.
 *
 * Identity is the only decision in onboarding that contaminates everything
 * downstream — three Black Daves exist in the database, a fourth (a UK rapper)
 * and a Chord DAVE audio DAC exist on the web, and no amount of search quality
 * separates them. One human answer does, permanently. So it is asked alone,
 * first, and everything expensive waits behind it.
 *
 * Shows a photo and a TRACK NAME rather than a list of URLs: an artist
 * recognises their own song instantly and would have to squint at a Spotify ID.
 *
 * "Not quite" is a first-class path, not an error — it opens the full profiles
 * card, which is the right tool for genuinely ambiguous identity.
 */
export function IdentityCard({ payload, onAnswer, disabled }: {
    payload: IdentityPayload;
    onAnswer: (isMe: boolean) => void;
    disabled: boolean;
}) {
    const followers = typeof payload.followerCount === "number" && payload.followerCount > 0
        ? payload.followerCount.toLocaleString()
        : null;

    return (
        <div className="glass p-5 space-y-4 rounded-xl">
            <div className="flex items-center gap-4">
                {payload.imageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element -- remote artist art; next/image would need a domain allowlist
                    <img
                        src={payload.imageUrl}
                        alt=""
                        referrerPolicy="no-referrer"
                        className="w-20 h-20 rounded-full object-cover flex-shrink-0 bg-black/5 dark:bg-white/10"
                    />
                ) : (
                    <div className="w-20 h-20 rounded-full flex-shrink-0 bg-gradient-to-br from-pastypink/30 to-purple-500/20" />
                )}
                <div className="min-w-0">
                    <h3 className="text-xl font-bold text-black dark:text-white truncate">{payload.artistName}</h3>
                    {payload.topTrack && (
                        <p className="text-sm text-gray-700 dark:text-gray-300 truncate">
                            Known for <span className="font-medium">&ldquo;{payload.topTrack}&rdquo;</span>
                        </p>
                    )}
                    <p className="text-xs text-gray-600 dark:text-gray-400">
                        {[
                            followers ? `${followers} followers` : null,
                            payload.releaseCount > 0 ? `${payload.releaseCount} release${payload.releaseCount === 1 ? "" : "s"}` : null,
                            payload.platform ? `on ${payload.platform}` : null,
                        ].filter(Boolean).join(" · ")}
                    </p>
                </div>
            </div>

            <p className="text-base font-semibold text-black dark:text-white">Is this you?</p>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                ...
                <button
                    ...
                    disabled={disabled}
                    className="flex-1 text-sm py-2.5 rounded-lg border border-pink-500 text-pink-500 enabled:hover:bg-pink-500/10 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                    Not quite
                </button>
            </div>
            <p className="text-xs text-gray-600 dark:text-gray-400">
                Saying yes lets us build your page from here — you can change anything afterwards.
            </p>
        </div>
    );
}
