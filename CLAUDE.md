...
- These policies are `USING (true)` / `WITH CHECK (true)` — they only gate **"is this the `mnweb` role at all,"** not per-row or per-user access. **All real authorization lives in application code** (`requireAuth`, `requireAdmin`, `verifyArtistEditable`, etc.); RLS here is not row-level user isolation.
